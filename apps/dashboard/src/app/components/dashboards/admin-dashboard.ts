      ...
      <header class="bg-blue-600 text-white shadow-lg">
        <div class="max-w-7xl mx-auto px-4 py-4 flex justify-between items-center">
          <div>
            ...
            <p class="text-blue-200">Task Management • {{ user()?.email }}</p>
          </div>
          <button (click)="logout()" class="px-4 py-2 bg-blue-800 rounded hover:bg-blue-900">Logout</button>
        </div>
      </header>
      ...
      <main class="max-w-7xl mx-auto px-4 py-6">
        ...
        <div class="flex gap-2 mb-6 border-b border-gray-300">
          <button (click)="activeTab.set('tasks')" class="px-4 py-2 font-medium"
                  [class]="activeTab() === 'tasks' ? 'border-b-2 border-blue-500 text-blue-600' : 'text-gray-600'">
            Tasks
          </button>
          <button (click)="activeTab.set('audit')" class="px-4 py-2 font-medium"
                  [class]="activeTab() === 'audit' ? 'border-b-2 border-blue-500 text-blue-600' : 'text-gray-600'">
            Audit Log
          </button>
        </div>

        <div *ngIf="activeTab() === 'tasks'">
          <button (click)="showModal.set(true)" class="mb-4 px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600">
            Add Task
          </button>
...
export class AdminDashboardComponent implements OnInit {
...
} = { title: '', description: '', category: 'work', status: 'todo' };
 ...
 editTask(task: any) {
  ...
  this.taskForm = { 
    ...
    category: task.category as 'work' | 'personal' | 'urgent' | 'other', 
...
closeModal() {
  ...
  this.taskForm = { title: '', description: '', category: 'work', status: 'todo' };
}
